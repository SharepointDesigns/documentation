import { themes as prismThemes } from "prism-react-renderer";
import type { Config } from "@docusaurus/types";
import type * as Preset from "@docusaurus/preset-classic";

// Staging configuration for test branch
const config: Config = {
  title: "SharePoint Designs Documentation (STAGING)",
  tagline: "Documentation for SharePoint Designs - Test Environment",
  url: "https://sharepointdesigns.github.io",
  baseUrl: "/documentation-staging/",
  favicon:
    "data:image/png;base64,",

  // GitHub pages deployment config for staging
  organizationName: "SharePointDesigns",
  projectName: "documentation",
  deploymentBranch: "gh-pages-staging",
  trailingSlash: false,
  onBrokenLinks: "warn", // More lenient for staging
  onBrokenMarkdownLinks: "warn",

  i18n: {
    defaultLocale: "en",
    locales: ["en"],
  },

  presets: [
    [
      "classic",
      {
        docs: {
          sidebarPath: "./sidebars.ts",
          editUrl:
            "https://github.com/SharepointDesigns/documentation/edit/test/spd-documentation/",
        },
        blog: {
          showReadingTime: true,
          feedOptions: {
            type: ["rss", "atom"],
            xslt: true,
          },
          editUrl:
            "https://github.com/SharepointDesigns/documentation/edit/test/spd-documentation/",
          onInlineTags: "warn",
          onInlineAuthors: "warn",
          onUntruncatedBlogPosts: "warn",
        },
        theme: {
          customCss: "./src/css/custom.css",
        },
      } satisfies Preset.Options,
    ],
  ],

  themeConfig: {
    // Add staging banner
    announcementBar: {
      id: "staging",
      content:
        '⚠️ This is a STAGING environment. For production docs, visit <a target="_blank" href="https://sharepointdesigns.github.io/documentation/">here</a>',
      backgroundColor: "#ff6b35",
      textColor: "#fff",
      isCloseable: false,
    },
    image: "img/spdlogo.svg",
    navbar: {
      title: "Documentation (STAGING)",
      logo: {
        alt: "SharePoint Designs Logo",
        src: "img/spdlogo.svg",
      },
      items: [
        {
          href: "https://github.com/sharepointdesigns",
          label: "GitHub",
          position: "right",
        },
      ],
    },
    footer: {
      style: "dark",
      links: [
        {
          title: "Docs",
          items: [
            {
              label: "Documentation",
              to: "/docs/intro",
            },
          ],
        },
        {
          title: "Community",
          items: [
            {
              label: "Linkedin",
              href: "https://www.linkedin.com/company/sharepoint-designs/",
            },
            {
              label: "Blog",
              to: "https://www.sharepointdesigns.com/blog",
            },
            {
              label: "GitHub",
              href: "https://github.com/sharepointdesigns",
            },
          ],
        },
        {
          title: "Support",
          items: [
            {
              label: "📧 sales@sharepointdesigns.com",
              to: "mailto:sales@sharepointdesigns.com",
            },
            {
              label: "📞 +91 9884189463",
              to: "tel:+919884189463",
            },
          ],
        },
      ],
      copyright: `Copyright © ${new Date().getFullYear()} SharePoint Designs. STAGING ENVIRONMENT`,
    },
    prism: {
      theme: prismThemes.github,
      darkTheme: prismThemes.dracula,
    },
  } satisfies Preset.ThemeConfig,
};

export default config;
